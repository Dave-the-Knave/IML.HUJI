...
class Perceptron(BaseEstimator):
    ...
    def __init__(self,
        ...
        self.coefs_ = None
        self.training_loss_ = np.array(np.zeros([self.max_iter_]), dtype=float)
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for i in range(self.max_iter_):
            # get index of misclassified sample, -1 if none
            cls = np.matmul(X, self.coefs_)
            misclassified_sample = next(iter(np.where(cls * y <= 0)[0]), -1)
            if misclassified_sample == -1:
                break
            # update model
            self.coefs_ = self.coefs_ + (y[misclassified_sample] * X[misclassified_sample, :])
            self.fitted_ = True
            # callback to store intermediate losses
            self.training_loss_[i] = self.callback_(self, X, y)
        return
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        classification = np.sign(np.matmul(X, self.coefs_))
