...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        # initialize intercept and weights
        if self.include_intercept_ == True:
            X = np.insert(X, 0, np.ones((X.shape[0], )), axis=1)
        if self.coefs_ is None:
            self.coefs_ = np.random.normal(0, 1/X.shape[1], X.shape[1])
        # implement fit depending on regularization
        gd = self.solver_
        if self.penalty_ == "none":
            self.coefs_ = gd.fit(f=LogisticModule(self.coefs_), X=X, y=y)
        else:
            if self.include_intercept_ == True:
                reg_param = np.delete(self.coefs_, 0)
            else:
                reg_param = self.coefs_
            if self.penalty_ == "l1":
                regularizer = L1
                ...
                raise ValueError("Supported penalty types are: none, l1, l2")
            self.coefs_ = gd.fit(f=RegularizedModule(fidelity_module=LogisticModule(self.coefs_),
                                                    regularization_module=regularizer(reg_param),
                                                    lam=self.lam_,
                                                    weights=self.coefs_,
